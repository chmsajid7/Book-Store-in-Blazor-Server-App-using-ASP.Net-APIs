...
namespace BlazorServerApp.Services
{
    public class BookStoresService<T> : IBookStoresService<T>
    {
        ...
        public async Task<ResponseDTO<T>> GetAllAsyncWithResult(string requestUri,string valueParamName)
        {
            var requestMessage = new HttpRequestMessage(HttpMethod.Get, requestUri);
            ...
            if (responseStatusCode.ToString() == "OK")
            {
                ...
                {
                    resultList = await Task.FromResult(JsonConvert.DeserializeObject<List<T>>(responseBody)),
                    resultValue = int.Parse(response.Headers.GetValues(valueParamName).FirstOrDefault())
                };
            }
            else
                return null;
